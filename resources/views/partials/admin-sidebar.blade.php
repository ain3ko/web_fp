...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 md:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-white">
      <ul class="space-y-2 font-medium">
         <li>
            <a href="/admin/admin-beranda" class="flex items-center p-2 text-gray-900 rounded-lg">
         ...
         <li>
            <a href="/admin/admin-tambah" class="flex items-center p-2 text-gray-900 rounded-lg">
               ...
               <span class="flex-1 ms-3 whitespace-nowrap">Tambah</span>
            </a>
         </li>
         <li>

            <form action="{{ route('logout') }}" method="POST" role="menuitem" class="flex items-center p-2">
            @csrf
            <div>
            ...
            <div><input type="hidden" name="_token" value="{{ request()->header('X-CSRF-TOKEN') ?: csrf_token() }}">
            <button type="submit" class="block px-4 py-2 text-sm text-gray-700">Log out</button></div>
        </form>
         </li>
      </ul>
   </div>
